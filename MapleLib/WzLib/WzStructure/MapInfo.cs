    ...
    public class MapInfo //Credits to Bui for some of the info
    {
        ...
        public void Save(WzImage dest, Rectangle? VR)
        {
            ...
            if (autoLieDetector != null)
            {
                var prop = new WzSubProperty
                {
                    ["startHour"] = InfoTool.SetOptionalInt(autoLieDetector.Value.startHour),
                    ["endHour"] = InfoTool.SetOptionalInt(autoLieDetector.Value.endHour),
                    ["interval"] = InfoTool.SetOptionalInt(autoLieDetector.Value.interval),
                    ["prop"] = InfoTool.SetOptionalInt(autoLieDetector.Value.prop)
                };
